...
Hand-written selectors after the `@import` are kept to a minimum and only where Bootstrap has no equivalent. Existing ones: `.tracking-wide`, `h2` / `.h2` (fluid `clamp()` section-title size — overrides Bootstrap's RFS step-scaling), `.hero-image`, `.bg-hero` (the `#top` radial-glow backdrop — stops derived from `$primary` / `$dark`), the `#mainNav` scroll-aware chrome (transparent over the hero at the top; solid `$dark` + hairline border once `.scrolled` is added by JS, and always solid below `lg` — Bootstrap's navbar has no such state), the `[data-icon-tile]` 48×48 sizing (no Bootstrap fixed-square utility), the `#projects .card` hover (subtle 4px lift + hover-blue border + `$box-shadow-sm` — no Bootstrap card hover state), and the `#projectStripDesktop` / `#projectStripDots` rules (see JavaScript below). Match that pattern if a genuinely missing utility is unavoidable; otherwise compose existing utilities.
...
- `assets/js/main.js` holds four small bespoke behaviors: the `#contactForm` submit → prefilled `mailto:` to liandrejohn88@gmail.com, the `#backToTop` show/hide on scroll past the hero, the `#projectStripDesktop` scroll buttons above, and the `#mainNav` `.scrolled` toggle (added once `window.scrollY > 4`, drives the navbar background — see the `#mainNav` rule in `main.scss`).
